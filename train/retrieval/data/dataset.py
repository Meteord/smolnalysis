...
def load_json_samples(path: str | Path) -> list[dict[str, Any]]:
    """Load retrieval SFT samples from one JSON file, a JSONL file, or a split directory."""
    path = Path(path)
    if path.is_dir():
        rows = []
...
def extract_retrieval_messages(sample: dict[str, Any]) -> list[dict[str, str]]:
    """Return normalized chat messages for one question-to-tool-result sample."""
    messages = sample.get("messages")
    if not isinstance(messages, list):
        raise ValueError("Retrieval sample must include messages.")
    ...
    if len(assistant_messages) != 1:
        raise ValueError("Retrieval samples must include exactly one assistant target.")

    user_messages = [message for message in normalized if message["role"] == "user"]
    if len(user_messages) != 1:
        raise ValueError("Retrieval samples must include exactly one user message.")

    if TOOL_RESULT_MARKER in user_messages[0]["content"]:
        raise ValueError("User prompt must not contain the source tool-result marker.")
    if TOOL_RESULT_MARKER in assistant_messages[0]["content"]:
        raise ValueError("Assistant label must not contain the source tool-result marker.")

    try:
        json.loads(assistant_messages[0]["content"])
    except json.JSONDecodeError as exc:
        raise ValueError("Assistant label must be valid JSON.") from exc
...
class ToolResultTrainingDataset:
    """Dataset for supervised question-to-tool-result adapter training."""
    ...
    def __init__(
        ...
        *,
        ...
        if not self.samples:
            raise ValueError(f"No retrieval samples found in {self.data_path}")
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if self.include_metadata:
            ...
            features["metadata"] = sample.get("metadata") or {}
        return features


class ToolResultDataCollator:
    """Pad ToolResultTrainingDataset items into a causal-LM batch."""
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, Any]:
        try:
            ...
            raise ModuleNotFoundError("torch is required to collate retrieval training batches.") from exc
