...
def build_scenarios_for_package(endpoint: str, query: str, package: dict[str, Any], index: int, filters: dict[str, str] | None = None) -> list[dict[str, Any]]:
    ...
    scenarios.append(
        {
            **base,
            "id": f"{query}_{package_id}{suffix}_tag",
            "state": "No catalog tools have been run yet.",
            "observed_packages": [],
            "observed_resources": [],
            "observed_tags": [],
            "observed_groups": [],
            ...
            "target_action": "tag_search",
            "notes": "Start by discovering CKAN vocabulary for the user topic.",
        }
    )
    scenarios.append(
        {
            **base,
            ...
            "observed_packages": [],
            "observed_resources": [],
            "observed_tags": _tag_names(package),
            "observed_groups": _group_names(package),
            "observed_organizations": [_organization_name(package)] if _organization_name(package) else [],
            "tool_errors": [],
            "has_enough_evidence": False,
            "target_action": "package_search",
            "notes": "Use the user request plus any discovered tags/groups/organizations to search packages.",
        }
    )
    scenarios.append(
        {
    ...
    if tabular_resources:
        ...
        scenarios.append(
            {
                **base,
                "id": f"{query}_{package_id}{suffix}_retry",
                ...
                "observed_groups": _group_names(package),
                "observed_organizations": [_organization_name(package)] if _organization_name(package) else [],
                "tool_errors": [{"tool": "package_show", "summary": "Observed resources are not suitable for tabular analysis."}],
                "has_enough_evidence": False,
                "target_action": "package_search",
                "notes": "Retry retrieval with a different package_search query after unsuitable resources.",
            }
        )
...
def build_inventory_scenarios_for_package(endpoint: str, package: dict[str, Any], index: int) -> list[dict[str, Any]]:
    ...
    scenarios = [
        {
            **base,
            "id": f"inventory_{topic_slug}_{index}{suffix}_tag",
            "state": "No catalog tools have been run yet.",
            "observed_packages": [],
            "observed_resources": [],
            "observed_tags": [],
            ...
            "has_enough_evidence": False,
            "target_action": "tag_search",
            "notes": "Discover CKAN tags for the natural-language topic before package search.",
        },
        {
            **base,
            "id": f"inventory_{topic_slug}_{index}{suffix}_group",
            "state": "A tag search was empty or too broad. Discover catalog groups before searching packages.",
            "observed_packages": [],
            "observed_resources": [],
            "observed_tags": [],
            "observed_groups": [],
            "observed_organizations": [],
            "tool_errors": [{"tool": "tag_search", "summary": "No useful tags found."}],
            "has_enough_evidence": False,
            "target_action": "group_list",
            "notes": "Use group_list after weak tag discovery.",
        },
        {
            **base,
            "id": f"inventory_{topic_slug}_{index}{suffix}_org",
            "state": "Tag/group discovery was broad. Discover organizations before searching packages.",
            "observed_packages": [],
            "observed_resources": [],
            "observed_tags": _tag_names(package)[:3],
            "observed_groups": _group_names(package)[:3],
            "observed_organizations": [],
            "tool_errors": [{"tool": "package_search", "summary": "Previous package_search returned broad candidates."}],
            "has_enough_evidence": False,
            "target_action": "organization_list",
            "notes": "Use organization_list to narrow broad catalog results.",
        },
        {
            **base,
            ...
            "has_enough_evidence": False,
            "target_action": "package_search",
            "notes": _policy_hint(filters, "Use discovered catalog vocabulary, groups, organizations, and the user request to search packages."),
        },
        {
            **base,
    ...
    if (unsuitable_resources and not tabular_resources) or (tabular_resources and not topic_matches_package):
        ...
        scenarios.append(
            {
                **base,
                "id": f"inventory_{topic_slug}_{index}{suffix}_retry",
                ...
                "observed_groups": _group_names(package),
                "observed_organizations": [_organization_name(package)] if _organization_name(package) else [],
                "tool_errors": [{"tool": "package_show", "summary": "Observed resources are unsuitable or candidate is too broad."}],
                "has_enough_evidence": False,
                "target_action": "package_search",
                "notes": "Retry with a refined package_search query instead of selecting a broad or unsuitable candidate.",
            }
        )
    return scenarios
